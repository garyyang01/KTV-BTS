...
#### 步驟 1.1：建立主頁面結構
- [ ] 建立 `lib/pages/main_page.dart`
- [ ] 設計頁面基本佈局（搜索區域、內容區域）
- [ ] 實作響應式設計架構
- [ ] 添加頁面標題和基本樣式

#### 步驟 1.2：實作搜索組件
- [ ] 建立 `lib/widgets/search_bar_widget.dart`
- [ ] 實作可輸入的下拉選單 (Autocomplete/ComboBox) 包含：
  - Munich Central (車站選項)
...
#### 步驟 3.3：完善導航邏輯
- [ ] 更新 `main.dart` 的路由配置
- [ ] 替換 `TicketBookingPage` 為新的 `MainPage` 作為首頁
- [ ] 保留現有的支付頁面和確認頁面路由
- [ ] 實作頁面間的導航邏輯
- [ ] 添加返回和重置功能
