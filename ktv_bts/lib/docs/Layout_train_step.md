...
### ⏳ 待開發階段：
- **階段一：UI 組件設計** - 火車票彈出頁面設計
- **階段二：資料模型建立** - TrainTicket 模型類別
- **階段三：流程整合** - 與現有購票流程整合
...
#### 步驟 1.1：建立火車票彈出頁面結構
- [ ] 建立 `lib/pages/train_ticket_selection_page.dart`
- [ ] 設計 Modal Bottom Sheet 或 Dialog 彈出介面
- [ ] 實作頁面基本佈局（標題、內容區域、按鈕區域）
- [ ] 添加關閉/返回功能

#### 步驟 1.2：設計火車票卡片組件
- [ ] 建立 `lib/widgets/train_ticket_card.dart`
- [ ] 設計票券卡片 UI 包含：
  - 起始站 → 終點站 路線顯示
  - 車種名稱和類型
  - 出發/到達時間
...
#### 步驟 1.3：實作火車票列表組件
- [ ] 建立 `lib/widgets/train_ticket_list.dart`
- [ ] 實作多張火車票的列表顯示
- [ ] 添加單選功能（一次只能選一張火車票）
- [ ] 實作空狀態顯示（無可用火車票時）
- [ ] 添加載入狀態指示器
...
#### 步驟 1.4：設計總金額顯示區域
- [ ] 建立 `lib/widgets/order_summary_widget.dart`
- [ ] 實作訂單摘要顯示包含：
  - 城堡門票總價
  - 選中的火車票價格（如有）
  - 總金額計算和顯示
...
#### 步驟 2.1：建立火車票資料模型
- [ ] 建立 `lib/models/train_ticket.dart`
- [ ] 定義 TrainTicket 類別包含所有必要欄位
- [ ] 實作 JSON 序列化/反序列化方法
- [ ] 添加資料驗證邏輯
